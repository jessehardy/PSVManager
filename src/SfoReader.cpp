...
bool SfoReader::load(const char* key_offset)
{
	...
	for (uint32_t i = 0; i < header->pair_count; i++) {
		...
		const char *value = key_offset + header->value_offset + index[i].data_offset;
		data.insert(std::make_pair(std::string(curr_key), std::string(value)));
	}
